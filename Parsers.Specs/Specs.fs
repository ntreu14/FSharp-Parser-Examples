...
let tests =
  testList "FParsec specs" [
    ...
    testList "simple parsers" [
      testCase "parsing a simple float value" <| fun _ ->
        Expect.isTrue 
          (isSuccessful <| run parseFloat "1.55") 
          "returns a Success"

      testCase "parsing a string that isn't a float" <| fun _ -> 
        Expect.isTrue
          (isError <| run parseFloat "foobar")
          "returns a Error"

      testCase "parsing a float between brackets" <| fun _ ->
        Expect.isTrue
      ...
      testCase "parsing strings with missing brackets" <| fun _ ->
        let shouldBeAllErrors = 
          [ "[123.43"; "54.5]" ] |> List.forall (isError << run parseFloatBetweenBrackets)
        
        Expect.isTrue
          shouldBeAllErrors
          "returns Errors"

      testCase "parsing strings with brackets not at the start or end" <| fun _ ->
        let shouldBeAllErrors =
      ...
      testCase "parsing literally any other string string that is not 'foo' or 'bar'" <| fun _ ->
        Expect.isTrue
